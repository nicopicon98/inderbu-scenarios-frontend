...
export function EditSubScenarioDialog({ open, subScenario, onOpenChange }: Props) {
    const { scenarios, activityAreas, fieldSurfaceTypes, updateSubScenario } = useSubScenarioData();
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="w-[650px] max-h-[80vh] mx-auto overflow-y-auto">
                ...
                <SubScenarioForm value={form} onChange={setForm}
                    scenarios={scenarios} activityAreas={activityAreas}
                    fieldSurfaceTypes={fieldSurfaceTypes} />

                <DialogFooter className="pt-3">
                    <Button variant="outline" onClick={() => onOpenChange(false)} size="sm">Cancelar</Button>
                    <Button className="bg-teal-600 hover:bg-teal-700" size="sm" onClick={save}>Guardar</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    );
