...
export function useRoleChecks() {
  ...
  return {
    ...
    hasAnyRole: (roles: EUserRole[]) => isAuthenticated && user && roles.includes(user.role),
    isAtLeastRole: (minRole: EUserRole) => isAuthenticated && user && user.role <= minRole,
    ...
    // Checks compuestos
    isAnyAdmin: () => isAuthenticated && user && [EUserRole.SUPER_ADMIN, EUserRole.ADMIN].includes(user.role),
    isNonAdmin: () => isAuthenticated && user && ![EUserRole.SUPER_ADMIN, EUserRole.ADMIN].includes(user.role),
  };
}
